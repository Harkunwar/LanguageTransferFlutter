...
if [ "${BRANCH_NAME}" = "master" ]; then
  ...
  if BRANCH_BASE_SHA=$(git merge-base --fork-point FETCH_HEAD HEAD); then
    ...
    FLUTTER_CHANGED_GLOBAL=`git diff --name-only $BRANCH_BASE_SHA HEAD | grep -v / | wc -l`
    FLUTTER_CHANGED_PACKAGES=`git diff --name-only $BRANCH_BASE_SHA HEAD | grep -o "/[^/]*" | sed -e "s/\///g" | sort | uniq | paste -s -d, -`
  else
